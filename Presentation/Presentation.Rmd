...
- We used the urbnmapr package in order to create maps with US state and county outlines, and added layers so that only the Western US, our focus for this project, would be shown.

- We then filled by `arithmetic_mean`, the variable encoding average yearly pollution levels by county, using a continuous, self-created scale, or by Air Quality Category, a discrete variable that we created by taking direct inspiration from the globally standardized Air Quality Index categories, and using the once again globally standardized discrete color scale for the Air Quality Category chloropleth map.

- We used the tabPanel method to create multiple tabs in our app, also including tabs for line plots of pollutants and of Air Quality Index (the value which informs the Air Quality Categorization) over time, and a tab for the dataset itself, in a readable format.
