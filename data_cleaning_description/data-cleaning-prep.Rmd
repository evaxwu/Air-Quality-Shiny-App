...
|>
  summarise(AQI = max(Ozone_index,PM2.5_index,CO_index,NO2_index,SO2_index,PM10_index))
...
pollutant_standards <- tibble(unique(CA1$pollutant_standard)) |>
  rename(pollutant_standard = "unique(CA1$pollutant_standard)") |>
  filter(!str_detect(pollutant_standard, "Annual"))
...
# No need to include the below in proposal—just say that we will repeat the above code for each 10-year increment from 1971 to 2021, and for all 50 states plus DC
...
CA5 <- aqs_quarterlysummary_by_state(parameter = c("88502", "42101", "42602", "44201", "81102", "88101", "42401"), bdate = as.Date("19710101", format = "%Y%m%d"), edate = as.Date("19800101", format = "%Y%m%d"), "06")

unique(CA5$parameter)
...
CA_overall <- rbind(CA1_avg2,CA2_avg2,CA3_avg2,CA4_avg2,CA5_avg2) |>
  ...
  unique() |>
  group_by(year, state, state_code, county, county_code, latitude, longitude, pollutant, units_of_measure) %>%
  summarize(arithmetic_mean = mean(arithmetic_mean)) 
# since there are multiple stations measuring air quality, we take the mean of all so that each county has 
# one row for each pollutant in each year

write_csv(CA_overall, "CA_ovberall.csv")
